...
bool Parser::Table_level_hint::resolve(Parse_context *pc) const
{
          ...
          at_query_block_name_opt_table_name_list= *this)
  {
    ...
    if (qb == NULL)
      return false;
    if (false /* FIXME: at_query_block_name_opt_table_name_list.is_empty() */)
    {
      // e.g. BKA(@qb1)
...
{
  if (false /* FIXME: is_empty() */)  // Empty list of index names, no additional info
    return;
