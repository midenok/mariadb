...
public:
  ...
  void init(const Lex_ident_db _ref_db, const Lex_ident_table _ref_table,
            const LEX *lex);
...
public:
  ...
  inline Table_ident(THD *thd, const LEX_CSTRING *db_arg,
		     ...
		     bool force)
    :table(Lex_ident_table(*table_arg)), sel((SELECT_LEX_UNIT *)0)
  {
    if (!force && (thd->client_capabilities & CLIENT_NO_SCHEMA))
      db= Lex_ident_db(null_clex_str);
    else
  ...
  */
  ...
  Table_ident(LEX_CSTRING &db_arg, LEX_CSTRING &table_arg)
    :db(Lex_ident_db(db_arg)), table(Lex_ident_table(table_arg))
  {}
  Table_ident()
  {}
