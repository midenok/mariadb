...
static
void binlog_query(THD *thd, st_ddl_recovery *r)
{
  LEX_CSTRING save_db;
  /* Reuse old xid value if possible */
  if (!r->xid)
    r->xid= server_uuid_value();
  thd->binlog_xid= r->xid;
  update_xid(r->execute_entry_pos, thd->binlog_xid);

  mysql_mutex_unlock(&LOCK_gdl);
  save_db= thd->db;
  lex_string_set3(&thd->db, r->db.ptr(), r->db.length());
  (void) thd->binlog_query(THD::STMT_QUERY_TYPE, r->query.ptr(), r->query.length(),
                           TRUE, FALSE, FALSE, 0);
  thd->binlog_xid= 0;
  thd->db= save_db;
  ...
  case DDL_LOG_RENAME_ACTION:
  {
    ...
    case RENAME_PHASE_BINLOG:
    {
      ...
      if (recovery_state.query.length() && mysql_bin_log.is_open())
        binlog_query(thd, &recovery_state);
