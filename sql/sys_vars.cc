...
static Sys_var_uint Sys_binlog_gtid_pos_cache_sparse_factor(
       "binlog_gtid_pos_cache_sparse_factor", "Sparse factor S for binlog_gtid_pos_cache "
       "means skip caching each S events, recover the missing entries from binlog file",
       GLOBAL_VAR(opt_binlog_gtid_pos_cache_sparse_factor), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, UINT_MAX), DEFAULT(0), BLOCK_SIZE(1));
