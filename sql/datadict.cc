...
int TABLE_SHARE::fk_write_shadow_frm_impl(THD *thd, const char *shadow_path)
{
  const uchar * frm_src;
  uchar * frm_dst;
  uchar * pos;
                                                ...
                                                bool replace_flag)
{
  ...
  if (ERROR_INJECT("log_replace_delete_1"))
    return true;
...
error:
  ...
  if (ERROR_INJECT("log_replace_delete_2"))
    goto error;
  list->next_active_log_entry= next_active_log_entry;
  if (ddl_log_write_execute_entry(list->entry_pos, &execute_entry))
    goto error;
  if (ERROR_INJECT("log_replace_delete_3"))
    goto error;
  return false;
}


int FK_backup::fk_write_shadow_frm(THD *thd, ddl_log_info &log_info)
{
  ...
  if (log_info.dbg_fail &&
      (ERROR_INJECT("fk_write_shadow_frm")))
  {
    err= 10;
    goto write_shadow_failed;
  }
#endif
  err= s->fk_write_shadow_frm_impl(thd, shadow_path);
  if (err)
  {
#ifndef DBUG_OFF
write_shadow_failed:
...
#ifndef DBUG_OFF
  if (log_info.dbg_fail &&
      (ERROR_INJECT("fk_install_shadow_frm")))
  {
    return true;
  }
#endif
...
int FK_backup_storage::write_shadow_frms(THD *thd)
{
  int err;
#ifndef DBUG_OFF
  FK_backup *last= NULL;
  ...
  for (auto &bak: *this)
  {
    ...
    if (&bak.second == last)
      dbg_fail= true;
#endif
    if ((err= bak.second.fk_write_shadow_frm(thd, *this)))
      return err;
  }
  return 0;
}
