...
static bool write_log_drop_frm(ALTER_PARTITION_PARAM_TYPE *lpt,
                               DDL_LOG_STATE *drop_chain, uint flags)
{
  char path[FN_REFLEN + 1];
...
{
  return write_log_drop_frm(lpt, lpt->part_info, 0);
}
  ...
  */
  ...
  else if (alter_info->partition_flags & ALTER_PARTITION_CONVERT_OUT)
  {
    ...
    if (mysql_write_frm(lpt, WFRM_WRITE_CONVERTED_TO) ||
        ...
        (frm_install= true, false) ||
        ddl_log_link_chains(&chain_drop_backup, lpt->part_info) ||
        write_log_drop_frm(lpt, &chain_drop_backup, WFRM_BACKUP_ORIGINAL) ||
