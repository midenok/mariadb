...
static
bool
innobase_change_field_versioning_try(
	trx_t* trx,
	const ulint tableid,
	const char* name,
	ulint prtype)
{
	DBUG_ENTER("innobase_change_field_versioning_try");
...
static
bool
innobase_change_fields_versioning_try(
	const Alter_inplace_info* ha_alter_info,
	const ha_innobase_inplace_ctx* ctx,
	trx_t* trx,
	const TABLE* table)
{
	DBUG_ENTER("innobase_change_fields_versioning_try");
	...
	for (uint i = 0; i < table->s->fields; i++) {
		...
		const dict_table_t* table = ctx->new_table;
		const dict_col_t* col
		    = dict_table_get_nth_col(table, i - virtual_count);

		DBUG_ASSERT(!col->vers_sys_start() && !col->vers_sys_end());
		...
		if (innobase_change_field_versioning_try(
			trx, table->id, col->name(*table), new_prtype)) {
			DBUG_RETURN(true);
		}
	}
...
static
void
innobase_change_fields_versioning_cache(
	Alter_inplace_info*		ha_alter_info,
	const ha_innobase_inplace_ctx*	ctx,
	const TABLE*			table)
{
	...
	const char*		table_name)
{
	...
	if (innobase_change_fields_versioning_try(ha_alter_info, ctx, trx,
						  old_table)) {
		DBUG_RETURN(true);
	}
	...
	trx_t*			trx)
{
	...
	if (ha_alter_info->handler_flags & ALTER_COLUMN_UNVERSIONED) {
		innobase_change_fields_versioning_cache(ha_alter_info, ctx,
							table);
	}
