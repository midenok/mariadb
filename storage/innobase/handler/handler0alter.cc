	...
	dict_s_col_list*s_cols)
{
	...
	*n_add_fk = 0;
	...
	for (FK_info& fk_key: ha_alter_info->alter_ctx->foreign_keys) {
		...
		LEX_CSTRING t = innodb_convert_name(cs, fk_key.referenced_table,
						    t_name);
		LEX_CSTRING d = fk_key.referenced_db.str
			? innodb_convert_name(cs, fk_key.referenced_db, db_name)
		...
		if (!innobase_init_foreign(
			...
			my_error(
				...
				MYF(0),
				"FOREIGN KEY", fk_key.foreign_id.str);
			goto err_exit_unlock;
		}
